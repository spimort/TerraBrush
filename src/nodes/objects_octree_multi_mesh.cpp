                ...
                if (add && existingNodes.size() == 0) {
                    ...
                    if (_definition->get_randomRotation()) {
                        if (_definition->get_randomRotationMin().x != 0 || _definition->get_randomRotationMax().x != 0) {
                            xRotation = Math::deg_to_rad(Utils::getNextFloatWithSeed((x * 1000) + y, _definition->get_randomRotationMin().x, _definition->get_randomRotationMax().x));
                        }

                        if (_definition->get_randomRotationMin().y != 0 || _definition->get_randomRotationMax().y != 0) {
                            yRotation = Math::deg_to_rad(Utils::getNextFloatWithSeed((x * 1000) + y, _definition->get_randomRotationMin().y, _definition->get_randomRotationMax().y));
                        }

                        if (_definition->get_randomRotationMin().z != 0 || _definition->get_randomRotationMax().z != 0) {
                            zRotation = Math::deg_to_rad(Utils::getNextFloatWithSeed((x * 1000) + y, _definition->get_randomRotationMin().z, _definition->get_randomRotationMax().z));
                        }
                    }
