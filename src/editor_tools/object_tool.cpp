...
void ObjectTool::paint(TerrainToolType toolType, Ref<Image> brushImage, int brushSize, float brushStrength, Vector2 imagePosition) {
    ...
    if (
        ...
        (currentObject->get_definition()->get_strategy() == ObjectStrategy::OBJECTSTRATEGY_OCTREEMULTIMESHES && currentObject->get_definition()->get_lodMeshes().size() == 0 && currentObject->get_definition()->get_objectScenes().size() == 0) ||
