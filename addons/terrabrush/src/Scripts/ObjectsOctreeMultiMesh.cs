...
public partial class ObjectsOctreeMultiMesh : Node3D, IObjectsNode {
    ...
    [BindProperty] public int ObjectsIndex { get;set; }
    ...
    [BindProperty] public bool LoadInThread { get;set; }
    [BindProperty] public int DefaultObjectFrequency { get;set;}
    ...
    protected override void _Ready() {
        ...
        _defaultNoise = ResourceLoaderHelper.Load<Texture2D>("res://addons/terrabrush/Resources/DefaultNoise.tres");
    ...
    private void InitializeMeshesAndCollision() {
        if (Definition.LODMeshes?.Count > 0) {
            _multiMeshIntances = new();

            for (var i = 0; i < Definition.LODMeshes.Count; i++) {
        ...
        } else if (Definition.ObjectScenes?.Count > 0) {
            for (var i = 0; i < Definition.ObjectScenes.Count; i++) {
                ...
                if (meshInstance != null) {
                    if ((Definition.LODList?.Count).GetValueOrDefault() == 0) {
                        var multiMeshInstance = new MultiMeshInstance3D() {
                            Multimesh = new MultiMesh() {
                                TransformFormat = MultiMesh.TransformFormatEnum.Transform3D,
                        ...
                        for (var lodIndex = 0; lodIndex < Definition.LODList.Count; lodIndex++) {
                            var lodDefinition = Definition.LODList[lodIndex];

                            var resultLodMesh = meshInstance.Mesh;
                            if (lodIndex != 0 && lodDefinition.AutomaticLOD) {
        ...
        for (var meshIndex = 0; meshIndex < multiMeshNodes.Count; meshIndex++) {
            ...
            for (var lodIndex = 0; lodIndex < multiMeshNode.Count; lodIndex++) {
                ...
                if (multiMeshNodeBuffer.Count == 0) {
                    ...
                    CallDeferred((StringName)nameof(AssignMultiMesheInstances), new ReadOnlySpan<Variant>([multiMeshInstance.Multimesh, new PackedFloat32Array(multiMeshNodeBuffer)]));
                }
            }
        }
    }

    private void AssignMultiMesheInstances(MultiMesh multiMesh, PackedFloat32Array instances) {
        multiMesh.InstanceCount = instances.Count / 12;
