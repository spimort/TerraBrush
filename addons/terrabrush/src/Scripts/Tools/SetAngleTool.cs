...
public class SetAngleTool : ToolBase {
    ...
    private void UpdateInitialPointMesh() {
        if (_setAngleInitialPoint == null) {
            ...
            if (_initialPointMesh == null) {
                _initialPointMesh = ResourceLoaderHelper.Load<PackedScene>("res://addons/terrabrush/Components/SetAngleInitialPoint.tscn").Instantiate<Node3D>();

                var container = _terraBrush.GetNodeOrNull(new NodePath(StringNames.SetAnglePointContainer.ToString()));
                if (container == null) {
    ...
    private void ClearInitialPointMesh() {
        var existingPointContainer = _terraBrush.GetNodeOrNull(new NodePath(StringNames.SetAnglePointContainer.ToString()));
