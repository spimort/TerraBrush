...
public partial class TerraBrushPlugin : EditorPlugin {
    ...
    private void CreateCustomSetting(string name, Variant defaultValue, VariantType type, PropertyHint hint = PropertyHint.None, string hintString = null) {
        if (ProjectSettings.Singleton.HasSetting(name)) {
            return;
        }

        var propertyInfo = new Godot.Collections.GodotDictionary() {
            {"name", name},
            {"type", (int)type},
            {"hint", (int)hint},
    ...
    protected override void _EnterTree() {
        ...
        CreateCustomSetting(SettingContants.IconsColor, Color.FromHtml("#00151F"), VariantType.Color);
    ...
    private void HandleKeyBindings() {
        var dlg = ResourceLoaderHelper.Load<PackedScene>("res://addons/terrabrush/Components/KeybindSettings.tscn")
    ...
    private Vector3 GetRayCastWithTerrain(Camera3D editorCamera) {
        ...
        if (editorCamera.GetViewport() is SubViewport viewport && viewport.GetParent() is SubViewportContainer viewportContainer) {
            ...
            if (result?.Count > 0 && result["collider"].AsGodotObject() == _currentTerraBrushNode.Terrain?.TerrainCollider) {
    ...
    private void ShowToolPieMenu(StringName actionName) {
    ...
    private void ShowBrushNumericSelector(int minVale, int maxValue, Color widgetColor, int initialValue, Action<int> onValueSelected, StringName actionName) {
        var previewActionName = HideOverlaySelector();
