...
public partial class TerraBrush : TerraBrushTool {
    ...
    public ShaderMaterial CustomShader {
        ...
        set {
            ...
            if (value != null && value.Shader == null) {
                var defaultShader = ResourceLoaderHelper.Load<Shader>("res://addons/terrabrush/Resources/Shaders/heightmap_clipmap_shader.gdshader");
    ...
    [PropertyGroup("Foliage")]
    [BindProperty]
    public override GodotArray<FoliageResource> Foliages { get;set; }
    ...
    [BindProperty]
    public override GodotArray<ObjectResource> Objects { get;set; }
    ...
    [BindProperty]
    public override GodotArray<MetaInfoLayer> MetaInfoLayers { get;set; }
    ...
    protected async override void _Ready() {
        ...
        if (string.IsNullOrEmpty(DataPath)) {
            ...
            if (!string.IsNullOrWhiteSpace(scenePath)) {
                DataPath = scenePath.Replace(Path.GetFileName(scenePath), GetTree().EditedSceneRoot.Name.ToString());
            }
        }
    ...
    private async Task CreateFoliages() {
        if (Foliages == null || Foliages.Count == 0) {
            return;
        }
    ...
    private void CreateMetaInfo() {
        if (MetaInfoLayers == null || MetaInfoLayers.Count <= 0) {
            return;
        }
    ...
    public TerrainPositionInformation GetPositionInformation(float x, float y) {
        ...
        if (zone != null) {
            ...
            if (MetaInfoLayers?.Count > 0 && zone.MetaInfoTexture != null) {
                ...
                if (metaInfoColorIndex >= 0 && MetaInfoLayers.Count - 1 >= metaInfoColorIndex) {
