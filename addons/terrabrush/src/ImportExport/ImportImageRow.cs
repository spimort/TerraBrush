...
public partial class ImportImageRow : PanelContainer {
    ...
    [BindProperty] public string ImageTypeName { get;set; }
    [BindProperty] public Texture2D ImageTexture { get;set; }
        ...
        _textureButton.GuiInput += async e => {
            if (e is InputEventMouseButton mouseEvent && !e.IsEcho() && !_isSelectorOpen) {
                if (mouseEvent.ButtonIndex == MouseButton.Left) {
                    ...
                    if (file != null) {
                        var imageResource = ResourceLoaderHelper.Load<Texture2D>(file);
