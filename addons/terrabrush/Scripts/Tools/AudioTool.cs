...
public class AudioTool : ToolBase {
    private Dictionary<ImageTexture, Image> _audioSplatImagesCache = null;

    public AudioTool(TerraBrush terraBrush) : base(terraBrush) {}

    public override void BeginPaint () {
        base.BeginPaint();

        _audioSplatImagesCache = new Dictionary<ImageTexture, Image>();
    }

    public override void EndPaint () {
        base.EndPaint();

        _audioSplatImagesCache = null;
    }
    ...
    public override void Paint(TerrainToolType toolType, Image brushImage, int brushSize, float brushStrength, Vector2 imagePosition) {
        if (_terraBrush.AudioIndex == null) {
            return;
        }

        var splatmapIndex = Mathf.FloorToInt(_terraBrush.AudioIndex.Value / 4);
        var colorChannel = _terraBrush.AudioIndex.Value % 4;
        var numberOfSplatmaps = Mathf.CeilToInt(_terraBrush.AudioCollection.AudioCollection.Count() / 4.0);
        ...
        ForEachBrushPixel(brushImage, brushSize, imagePosition, (imageZoneInfo, pixelBrushStrength) => {
            for (int i = 0; i < numberOfSplatmaps; i++) {
                ...
                var currentSplatmapTexture = imageZoneInfo.Zone.AudioTexture[i];
                ...
                _audioSplatImagesCache.TryGetValue(currentSplatmapTexture, out Image currentSplatmapImage);

                if (currentSplatmapImage == null) {
                    currentSplatmapImage = currentSplatmapTexture.GetImage();
                    _audioSplatImagesCache.Add(currentSplatmapTexture, currentSplatmapImage);
                }
        ...
        _terraBrush.TerrainZones.UpdateAudioSplatmapsTextures();
    }
}
#endif
