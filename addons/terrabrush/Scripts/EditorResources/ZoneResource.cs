...
public partial class ZoneResource : Resource {
	...
	[Export] public ImageTexture[] AudioTexture { get;set; }
	...
	public void InitializeImagesForTerrain(TerraBrush terraBrush) {
		...
		var numberOfAudiomaps = Mathf.CeilToInt((terraBrush.AudioCollection?.AudioCollection?.Length ?? 0) / 4.0f);
		var audiomaps = new List<ImageTexture>();
		for (var i = 0; i < numberOfAudiomaps; i++) {
			audiomaps.Add(ZoneUtils.CreateSplatmapImage(terraBrush.ZonesSize, ZonePosition, i, terraBrush.DataPath));
		}
		AudioTexture = audiomaps.ToArray();

  //      if (terraBrush.AudioCollection != null) {
		//	AudioTexture = terraBrush.AudioCollection.Select((objectItem, index) => {
